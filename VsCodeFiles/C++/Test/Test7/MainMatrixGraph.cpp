...
//深度优先遍历生成树(生成顶点v所在联通分量的树)
void DFSpainningTree(Graph G, int v, Tree &T, int parent){
	...
	for(int i = 1; i <= G.VerNum; i++){
		if (G.AdjMatrix[v][i] >= 1 && G.AdjMatrix[v][i] < INF && !visited[i]){
			DFSpainningTree(G, i, T, parent);
		}
	}
}

//广度优先遍历生成树(生成顶点v所在联通分量的树)
void BFSpainningTree(Graph G, int v, Tree &T, int parent){
	queue<int> vQueue;
	T.tree[T.numNode].data = G.Data[v];
	T.tree[T.numNode].parent = parent;
	parent = T.numNode++;
	...
	while(!vQueue.empty()){
		for(int i = 1; i <= G.VerNum; i++){
			if (G.AdjMatrix[temp][i] > 0 && G.AdjMatrix[temp][i] < INF && !visited[i]){
				//访问当前顶点的连接点
				//并将连接点入队
				T.tree[T.numNode].data = G.Data[i];
				T.tree[T.numNode].parent = parent;
				T.numNode++;
				visited[i] = true;
				vQueue.push(i);
...
bool IsTree(Graph G, int v0){
	...
	for(int i = 1; i <= G.VerNum; i++){
		//深度优先遍历v0的所有相连顶点
		if (G.AdjMatrix[v0][i] > 0 && G.AdjMatrix[v0][i] < INF && !visited[i]){
			IsTree(G, i);
			//这里相当于深度优先遍历
		}
	}
	...
	for(int i = 1; i <= G.VerNum; i++){
		/*
			若经过从v0开始的深度优先遍历后，图中的所有顶点都已被访问
			则此有相图为一棵以顶点v0为根结点的有向树
			否则，只要有一个顶点未被访问，此图都不是一棵以顶点v0为根结点的有向树
		*/
		if (!visited[i]){
			flag = false;
		}
...
void Menu(void){
	cout << "**************************************************************************" << endl;
    cout << "1.BFS(广度优先搜索遍历)" << endl;
    cout << "2.DFS(深度优先搜索遍历)" << endl;
    cout << "3.求给定图中的边（或弧）的数目" << endl;
    cout << "4.对给定的图G及出发点v0，设计算法从V0出发深度优先遍历图G，\n"
	         " 并构造出相应的生成树或生成森林" << endl;
    cout << "5.对给定的图G及出发点v0，设计算法从V0出发广度优先遍历图G，\n"
	  		 " 并构造出相应的生成树或生成森林。" << endl;
    cout << "6.实现Prim算法，求解下列给定图G的最小生成树" << endl;
    cout << "7.实现Kruskal算法，求解下列给定图G的最小生成树" << endl;
    cout << "8.实现Dijkstra算法，求解下列给定图G指定顶点到其余顶点之间的最短路径。" << endl;
	cout << "9.实现Floyd算法，求解下列给定图G各顶点之间的最短路径" << endl;
    cout << "10.设计算法求解下列给定图G的拓扑序列" << endl;
    cout << "11.设计算法求解下列给定AOE网的关键路径" << endl;
	cout << "**************************************************************************" << endl;
}

int main(void){
	Graph G1, G2, G3, G4, G5, G6;
	CreateGrpFromFile("TestData1.txt", G1);
	CreateGrpFromFile("TestData2.txt", G2);
	CreateGrpFromFile("TestData3.txt", G3);
	CreateGrpFromFile("TestData4.txt", G4);
	CreateGrpFromFile("TestData5.txt", G5);
	CreateGrpFromFile("TestData6.txt", G6);

	Tree T1, T2, T3, T4;

	int path1[MaxVerNum+1], dist1[MaxVerNum+1];
	int path2[MaxVerNum+1][MaxVerNum+1], dist2[MaxVerNum+1][MaxVerNum+1];
	int topologicalStr[MaxVerNum+1];
	...
	while(1){
        ...
        switch(testNum){
            ...
            case 2: DFS(G1, 1);
                    system("pause");
                    break;
            case 3: cout << "此图的边数为" << G1.ArcNum << endl;
                    system("pause");
                    break;
            case 4: DFSpainningTree(G2, 1, T1, -1);
					for(int i = 1; i <= G2.VerNum; i++){
						if (!visited[i]){
							DFSpainningTree(G2, i, T1, -1);
						}
					}
					T1.PreTraversal();
                    system("pause");
                    break;
            case 5: BFSpainningTree(G2, 1, T2, -1);
					for(int i = 1; i <= G2.VerNum; i++){
						if (!visited[i]){
							BFSpainningTree(G2, i, T2, -1);
						}
					}
					T2.PreTraversal();
                    system("pause");
            ...
            case 6: Prim(G3, T3, 1);
                    ...
                    system("pause");
                    break;
            case 7: KrusKal(G3, T4);
                    system("pause");
                    break;
            case 8: DijKstra(G4, path1, dist1, 1);
                    system("pause");
                    break;
            case 9: Floyd(G4, path2, dist2, 1);
                    system("pause");
                    break;
			case 10:TopologicalSort(G5, topologicalStr);
                    system("pause");
                    break;
			case 11:CriticalPath(G6, 1, 10);
                    system("pause");
                    break;
            case -1: return 0;
        }
		for(int i = 0; i < MaxVerNum+1; i++){
			visited[i] = false;
		}
        system("cls");
    }
